...
function update_totals(dandiset_id) {
    ...
    try {
        ...
        totals_element.innerText = `Totals ${human_readable_bytes_sent} sent from ${totals.number_of_unique_regions} regions across ${totals.number_of_unique_countries} countries.`;
    } catch (error) {
        console.error("Error:", error);
        if (totals_element) {
